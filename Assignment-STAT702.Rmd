...
my_reviews %>% 
  count(overall) %>% 
  mutate(percent = n/sum(n)*100) -> summary
```

## Answer

Reviews were rated from 1-5. The mean rating for product B00006IE7J is 
`r  summary(my_reviews$overall)[4]`. Most reviews were rated highly with `r (round(summary$percent[5], 1)`% given 5 and `r (round(summary$percent[4], 1)`% given 4 stars.
...
# Count frequency of words, excluding pen, pens, stars
tidy_sentiment_gc %>%
  count(reviewWord, sort = TRUE) %>% 
  filter(n > 100) %>% 
  mutate(reviewWord = reorder(reviewWord, n)) %>% 
...
bing_word_counts %>%
  ...
  top_n(10) %>%
  ungroup() %>%
  mutate(word = reorder(reviewWord, n)) %>%
