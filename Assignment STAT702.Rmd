...
Low performing stores could boost their sales by increasing the number of weeks they promote the product. For high performing stores, performance reliability could be improved by introducing a weekly promotion if sales are unusually low.
...
The EOQ model is used to find the best order quantity so that costs are minimised. There are three key assumptions to this model, these are that demand is constant and known, there is no lead time and no back orders are allowed. 

Demand for product 216233 has been estimated based on the annual demand from 2012, this is `r A`. Given this annual demand and the holding and set up costs the optimum order quantity is calculated to be `r Q` with an inventory cycle of `r t`. Optimum ordering quantity is caluclated using the following formula.

$  $


This means that every `r t` days `r Q` units are ordered. This model results in the smallest possible annual inventory cost of `r Tc`.
