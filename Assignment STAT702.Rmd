...
The expected annual demand is estimated to be `r round(D, 0)`. Given this annual demand and the costs of holding and reordering stock, the recommended multi-inventory model is to order `r Q` units whenever the order quantity reaches the reorder point of `r round(r, 0)` units. Approximately `r n` orders will be placed per year and safety stock is `r Q`. This approach ensures roughly 95% of the time stock will be sufficient for weekly demand. The expected annual costs are `r tac` per year. If demand was certain
the annual costs would only be `r tac2` so the additional cost of holding
safety stock is `r tac - tac2`.
...
• Descriptionsofresultsandplotsarecorrectandprovideusefulinsights.
• Plot/sareconstructedusingggplot2andhaveappropriatetitles,labels,scalesetc.


## Answe
First, we filter the reviews to show data only assigned to us. Asigned number: B00006IE7J
We are also checking the head of the filtered data to confirm that we have the correct data.
```{r 3a}
reviews_data %>%
...
Summarising the results, we get the following:
```{r}
summary(my_reviews$overall)
```


We plot the overall against the count to get a visual representation of the reviews. By flipping the co-ordinates the representation is similar to a representation of reviews on a website.
```{r 3a-plot}
# Stats - plots 
ggplot(data = my_reviews) +
  ...
  coord_flip()
```
The plot above shows us that the product is highly rated for 5 stars, way more than any other rating. The reviews are overall positive with a few negative reviews.
...
Finally, we plot the words against their occurance, based on their contributions to the sentiment. 
```{r plot_by_sentiment}
tidy_sentiment %>%
  inner_join(get_sentiments("bing"), by = c("reviewWord"  = "word")) %>% 
