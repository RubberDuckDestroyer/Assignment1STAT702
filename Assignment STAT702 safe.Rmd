...
sales_219844$category <- factor(sales_219844$category, levels = c("Featured & Displayed", "Featured", "Displayed", "None"))
...
# Bar plot showing monthly sales broken down into category
sales_219844 %>%
  group_by(month, category) %>% 
  summarise(total_units_sold = sum(units_sold)) %>% 
  ggplot() +
  geom_bar(aes(x = month, y = total_units_sold, fill = category), stat = "identity") +
...
# Scatter plot of weekly sales performance (top third ranking sotres)
sales_219844 %>% 
  filter(ranking < 18, ranking > 9) %>% 
  mutate(as.character(week)) %>% 
  ggplot()+
  geom_point(aes(x = week, y = units_sold, colour = performance)) + 
