...
ggplot(sales_summary) +
        ...
        xlab("Date") +
        ylab("Units Sold") +
        ggtitle("Monthly sales for product 219844")
...
sales_data %>% 
  ...
  arrange(date) %>% 
  ggplot() +
  geom_boxplot(aes(x = date, y = units_sold, group = date)) + 
  xlab("Date") +
  ylab("Units Sold") +
  ggtitle("Distribution of store monthly sales for product 219844")
...
sales_data %>% 
  ...
  arrange(date) %>% 
  ggplot() +
  geom_boxplot(aes(x = month, y = units_sold, group = month)) + 
  xlab("Date") +
  ylab("Units Sold") +
  ggtitle("Distribution of store monthly sales for product 219844")
...
quarterly_sales %>% 
  ...
  arrange(year) %>% 
  ggplot() +
  geom_bar(aes(x = store_id, y = total_units_sold, fill = year), stat = "sum") 

## Caluclate quartiles
#store_summary <- summary(store_sales$store_total)

## Rank stores into 4 groups based on total sales
#store_sales %>% 
#  mutate(ranking = ifelse(store_total < store_summary[2], "1", 
#                              (ifelse(store_total < store_summary[4], "2",
#                                      ifelse(store_total < store_summary[5], "3", #"4"))))) -> store_sales
