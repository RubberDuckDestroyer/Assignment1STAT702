...
# Uncomment code below to install required packages
...
of sku 219844 were sold with a monthly mean of `r round(summary(monthly_sales$total_units_sold)[4], 1)`. In the plots below monthly sales do not appear to follow a trend or seasonal pattern. Three months stand out with high sales, these are May 2011, December 2011 and January 2012. The most significant high sales occurred in January 2012 when `r outlier$total_units_sold` units were sold. As shown in the plots below these high monthly sales correspond with a high proportion of stores featuring and/or displaying the product, resulting in unusually high sales.

```{r 1a monthly sales plot, out.width = "60%"}
...
```{r test, figures-side, fig.show="hold", out.width="50%"}
...
`r store_total_sales$store_total[1]` units.  Below is a bar chart showing the total units sold with stores ordered by rank. Sales for 2013 are much lower than 2011 and 2012 as only half the
years data is included.


```{r 1b store totals plotted, out.width = "75%"}
...
As an alternative to ranking stores based on their overall sales, stores have been given a ranking based on their consistency. For every high performing month stores are given a score of 3, for every average month a score of 2 and for every low performing month a score of 1. The plot below shows the stores ordered by this new inconsistency ranking and their overall sales. Under the new consistency ranking scheme stores with unreliable performance  such as 9112 drop in rank whilst stores with reliable performance such as 9532 move up in rank. 
...
Below is a plot showing that when a product is featured and/or displayed weekly sales are more likely to be high. Weekly performance has been categorised into 'High', 'Average' and 'Low' based on the interquartile range of weekly sales. When the product is featured and/or displayed a higher proportion of weekly sales have been 'High' or 'Average' compared to no promotion. Low performing stores could boost their sales by increasing the number of weeks they promote the product and unreliable stores could ensure high sales by holding regular promotions.

```{r 1b promotion conditions, out.width="50%"}
...
```{r 2aii plot, out.width="75%", message = FALSE}
# Plotting
...
```{r 2bi plots,figures-side, fig.show="hold", out.width="50%"}
...
```{r 2bii lead-time distribution plot, figures-side, fig.show="hold", out.width="50%"}
# Lead-time distribution plot
x <- seq(from= 0,to=4000,len=100)
plot(x,dnorm(x,mean=mean,sd=sd),
     type="l",lwd=3,col="black", 
     main = "Lead-Time Distribution", 
     xlab = "Lead-Time Demand for 219844", 
     ylab = " ") +
  abline(v = r)
...
```{r actual distribution plot, figures-side, fig.show="hold", out.width="50%"}
# New distribution using interquartile range as standard deviation

# Calculate 95% threshold for new distribution
r1 <- qnorm(0.95, median(weekly_sales$quantity), sd=IQR(weekly_sales$quantity))

x <- seq(from= 0,to=1500,len=100)
plot(x,dnorm(x,mean=median(weekly_sales$quantity),sd=IQR(weekly_sales$quantity)),
     type="l",lwd=3,col="black", 
     main = "Lead-Time Distribution", 
     xlab = "Lead-Time Demand for 219844", 
     ylab = " ") +
...
ggplot() +
  geom_histogram(aes(x = quantity, y = ..density..),
               ...
               colour = "black") + 
  stat_function(fun = dnorm, 
              args = list(mean = median(weekly_sales$quantity), 
                          sd = IQR(weekly_sales$quantity))) +
  ggtitle("Lead-Time distributed plotted with actual distribution for 219844") +
  xlab("Quantity") +
  ylab("Density")
```
...
`r (round(summary$prop[5], 1))`% given 5 stars and `r (round(summary$prop[4], 1))`% given 4 stars.
...
```{r 3b word frequency, out.width = "50%"}
...
```{r 3b word sentiment, out.width= "75%"}
...
```{r 3b comparison,figures-side, fig.show="hold", out.width="50%"}
...
tidy_sentiment_gc %>% 
  ...
  ylab("Proportion")
```
The graphs below plot the top 100 negative and top 100 positive sentiment values against the actual overall values specified by the reviewers.
In the positive reviews, majority of the data is classified as an overall rating of 3 or above. A single outlier exists. The outlier has an overall value of 1 and a positive sentiment associated to it.
In the negative reviews, an outlier can be seen, where the overall rating is 5 but the sentiment given to the data is very negative. Additionally, a lot of data rated 5 stars has been given a negative sentiment. This can be attributed to the data not being balanced or the reviews having too many words. Both can affect the final analysis in different ways and must be considered as a way to improve sentiment analysis on the given data.

```{r top positive and negative sentiment scores, figures-side, fig.show="hold", out.width="50%"}
sentiment_summary %>% 
  slice_max(sentiment, n = 100) %>% 
  ggplot(aes(overall, sentiment)) +
  geom_point(position = "jitter") +
  ggtitle("Top 100 positive sentiment values") +
  xlab("Sentiment score") +
  ylab("Overall rating")

sentiment_summary %>% 
  slice_min(sentiment, n = 100) %>% 
  ggplot(aes(overall, sentiment)) +
  geom_point(position = "jitter") +
  ggtitle("Top 100 negative sentiment values") +
  xlab("Sentiment score") +
  ylab("Overall rating")
```
\newpage
...
**Hitarth 40%**
I contributed by solving question 1 and 3 in this project. I also compared my answers to my partner's question1 answers to make sure we had done the right thing. I initially tackled question 3 and asked Genevieve to help me out with beautifying some of the plots. I was happy to brainstorm solutions and ideas with my partner on anything we worked on. Finally, I was able to help add  attention to detail with the final formatting of our document.
