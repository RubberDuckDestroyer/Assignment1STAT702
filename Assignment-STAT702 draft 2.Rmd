...
# This explains spike, featured in heaps of stores in Jan 2012
sales_data %>% 
  filter(sku_id == 219844) %>%
  mutate(week = as_date(week, format = "%d/%m/%y"),
         as.character(week),
         category = ifelse(is_featured_sku == "1", "Featured", 
                              (ifelse(is_display_sku == "1", "Display", "None")))) %>% 
  ggplot() + 
...
sales_data %>% 
  ...
  mutate(week = as_date(week, format = "%d/%m/%y"),
         as.character(week),
         category = ifelse(is_featured_sku == "1" & is_display_sku == "1", "Featured & Displayed", ifelse(is_featured_sku == "1", "Featured", (ifelse(is_display_sku == "1", "Display", "None"))))) %>% 
  ggplot() + 
  geom_point(aes(x = week, y = units_sold, colour = category))
```

From Jan 2011 - July 2013 `r sum(monthly_sales$total_units_sold)` units of sku 219844 were sold with a mean monthly sale of 
`r mean(monthly_sales$total_units_sold)`. Over this period there was a significant outlier with `r outlier$total_units_sold` units sold on `r outlier$date`. 
...
# Order store_id variable based on ranking
x <- (store_sales$store_id)
store_sales$store_id_ranked <- factor(store_sales$store_id, levels = x)
```

```{r}
# Weekly sales performance classified into High, Average and Low based on interquartile range

# Weekly sales per store
sales_data %>% 
  filter(sku_id == 219844) %>%
  mutate(week = as_date(week, format = "%d/%m/%y"),
         store_id = as.character(store_id)) -> weekly_sales

# Classify weekly sales performance
weekly_sales %>% 
  mutate(performance = ifelse
         (units_sold < summary(weekly_sales$units_sold)[2], "Low", 
                              (ifelse
                               (units_sold > summary(weekly_sales$units_sold)[5],
                                 "High", 
                                 "Average")))) -> weekly_sales

# Add store ranking
weekly_sales %>% 
  left_join(store_sales, by = "store_id") -> weekly_sales 

# Add category
weekly_sales %>% 
  mutate(category = ifelse(is_featured_sku == "1" & is_display_sku == "1", "Featured & Displayed",ifelse(is_featured_sku == "1", "Featured",ifelse(is_display_sku == "1", "Displayed", "None")))) -> weekly_sales

# Order performance
weekly_sales$performance <- factor(weekly_sales$performance, levels=c("High", "Average", "Low"))

# Order category
weekly_sales$category <- factor(weekly_sales$category, levels = c("Featured & Displayed", "Featured", "Displayed", "None"))
```

```{r}
# Plot showing that high performing weeks are more likely to have display or featured product than average or low performing weeks

# Monthly sales broken down into category
weekly_sales %>%
  mutate(week = as_date(week, format = "%d/%m/%y"),
         date = format(week, "%m/%y"),
         date = my(date)) %>% 
  ...
  summarise(total_units_sold = sum(units_sold)) %>% 
  ggplot() +
  geom_bar(aes(x = date, y = total_units_sold, fill = category), stat = "identity")+
  scale_fill_manual(values = c("#D62246", "#4B1D3F", "#19D4D1", "#0E7C7B")) +
  ggtitle("Monthly sales for product 219844 with categories") +
  xlab("Date") +
  ylab("Units sold")
...
weekly_sales %>% 
  ggplot() +
  geom_bar(aes(x = category, fill = performance), position ="fill") +
   scale_fill_manual(values = c("#3c096c", "#7b2cbf", "#c77dff"))

weekly_sales %>%
  ggplot() +
  geom_point(aes(x = ranking, y= units_sold, colour = performance))

weekly_sales %>% 
  ggplot() +
  geom_bar(aes(x = ranking, fill = category))

# This explains spike, featured in heaps of stores in Jan 2012
weekly_sales %>% 
  mutate(as.character(week)) %>% 
  ggplot()+
  geom_point(aes(x = week, y = units_sold, colour = category))
...
```{r}
# Scatter plots showing weekly sales for each store along with category
